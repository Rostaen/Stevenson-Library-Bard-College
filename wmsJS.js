(function () {
    ...
    urlBase =
        "https://bardcollege.on.worldcat.org/external-search?queryString=#T#&databaseList=&clusterResults=on&groupVariantRecords=off&stickyFacetsChecked=on&baseScope=wz%3A624&sortKey=BEST_MATCH#F#";
    form.addEventListener("submit", function (e) {
        ...
        if (select) {
            ...
            if (index !== "kw")
                query = select.options[select.selectedIndex].value + ":" + query;
        }
        w.open(
            urlBase.replace("#T#", encodeURIComponent(query)).replace("#F#", f),
            rt
        );
    });
})();
